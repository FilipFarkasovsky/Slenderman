...
public class PlayerMovement : MonoBehaviour
{
    ...
    private float _wallRunUpForce = 20;
    ...
    private void OnCollisionStay(Collision other)
    {
        ...
        {
            if (readyToJump && jumping && isLeftWall)
            {
                if (readyToJump && jumping) Jump();
                Jump();
                //rb.AddForce(Vector3.up * _wallRunUpForce, ForceMode.Impulse);
                rb.AddForce(orientation.transform.right * _wallRunUpForce, ForceMode.Impulse);
            }
            if (readyToJump && jumping && isRightWall)
            {
                Jump();
